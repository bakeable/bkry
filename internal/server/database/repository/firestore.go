...
func (repo *FirestoreRepository[T]) Find(path string, queries []database.Query) (map[string]interface{}, error) {
    docs, err := firestore.QueryCollection(path, queries, database.Pagination{
		PageSize: 1,
		PageNumber: 1,
		OrderBy: "id",
		Ascending: true,
...
func (repo *FirestoreRepository[T]) Query(path string, queries []database.Query, pagination database.Pagination) ([]map[string]interface{}, error) {
    docs, err := firestore.QueryCollection(path, queries, pagination)
    if err != nil {
        return nil, err
    }
