...
func Decode(m interface{}) Product {
	if m, ok := m.(map[string]interface{}); ok {
		return Product{
			...
			PriceLayers: price_layer.DecodeAll(m["priceLayers"]),
...
func DecodeMargins(m interface{}) Margins {
	...
	if val, ok := m.(map[string]interface{}); ok {
		return Margins{
			Direct: margin.Decode(val["direct"]),
			Premium: margin.Decode(val["premium"]),
			Standard: margin.Decode(val["standard"]),
		}
	}
	return Margins{}
}
