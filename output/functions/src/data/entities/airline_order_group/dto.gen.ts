...
import { AirlineOrder } from '../airline_order'
import { AirlinePricing } from '../airline_pricing'
import { AirlineSettings } from '../airline_settings'
...
export class AirlineOrderGroupDto extends Dto implements IAirlineOrderGroupDto {
  ...
  override set(data: JSONData): void {
    ...
    if (data.orders !== undefined && data.orders !== null && Array.isArray(data.orders)) {
      this.orders = AirlineOrder.FromJSONArray(data.orders as JSONData[])
    }
    if (data.pricingTables !== undefined && data.pricingTables !== null && Array.isArray(data.pricingTables)) {
      this.pricingTables = AirlinePricing.FromJSONArray(data.pricingTables as JSONData[])
    }
    if (data.settings !== undefined && data.settings !== null ) {
      if (!this.settings) {
        this.settings = new AirlineSettings()
      }
